...
module.exports.loop = function () {
    ...
    /* Tower Defence  
     var tower = Game.getObjectById('2932342b34138d523fb292be');
     if(tower){
    ...
    */
    ...
    if(harvesters.length < 4){
        ...
        var newName = Game.spawns['Spawn1'].createCreep([WORK, CARRY,CARRY, MOVE, MOVE], ('u' + timeCreated), {role: 'upgrader'});
        console.log('Current upgrader count = ' + upgraders.length +
                    '\nSpawning new upgrader: ' + newName);
    }

    else if(builders.length < 4){
        var newName = Game.spawns['Spawn1'].createCreep([WORK, CARRY, CARRY, CARRY, MOVE, MOVE], ('b' + timeCreated), {role: 'builder'});
        console.log('Current builder count = ' + builders.length +
                    '\nSpawning new builder: ' + newName);
    }

    else if(repairers.length < 2){
        var newName = Game.spawns['Spawn1'].createCreep([WORK, CARRY, MOVE], ('r' + timeCreated), {role: 'repairer'});
